...
class StyleGAN(nn.Module):
    ...
    def gradient_penalty(self, real_input, fake_input):
        alpha = torch.rand(real_input.size(0),1,1,1).to(real_input.device)
        interpolates = alpha * real_input.detach() + (1 - alpha) * fake_input.detach()
    ...
    def forward_train(self, imgs, depth, alpha, **kwargs):
        ...
        disc_fake_output = self.discriminator(gen_output.clone().detach(), depth, alpha)
        # losses
        gradient_penalty = self.gradient_penalty(imgs, gen_output)
        losses['gen_loss'] = self.loss_gen(disc_fake_output)
        losses['disc_loss'] = self.loss_gen(disc_real_output, disc_fake_output, gradient_penalty=gradient_penalty)
        return losses
