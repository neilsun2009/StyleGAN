...
class StyleAdaIN(nn.Module):
    ...
    def forward(self, x, style_latent):
        ...
        style_w = self.affine_w(style_latent).view(-1, x.size(1), 1, 1)
        style_b = self.affine_b(style_latent).view(-1, x.size(1), 1, 1)
...
class StyleSynthesis(nn.Module):
    ...
    def __init__(self, in_channels, style_channels=512,
        ...
        blocks = [SynthesisBlock(nf(0), nf(0),
            style_channels, activation, is_first_block=True)]
