...
class MinibatchStddev(nn.Module):
    ...
    def forward(self, x):
        ...
        y = torch.mean([3, 4, 5], keepdim=True).squeeze(3)     
...
class ProGANDisc(nn.Module):
    ...
    def __init__(self, max_channels, resolution=1024, activation=None):
        ...
        for cur_depth in range(self.total_depth-1, 0, -1):
            ...
            blocks.append(DiscBlock(cur_channels, next_channels, activation))
        from_rgbs.append(EqualizedConv2d(3, nf(0), kernel_size=1, padding=0))
        blocks.append(DiscLastBlock(nf(0), nf(0), activation))
